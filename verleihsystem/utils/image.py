...
def entropy(image):
    """
    Calculate the entropy of a given image.

    Entropie ist ein Mass fuer die Informationsdichte (mittlerer Informations-
    gehalt) eines Zeichensatzes, wobei hier ein Pixel ein Zeichen ist.

    > http://www.reddit.com/r/pics/comments/j1q8q/for_science/
    ...
    represents the range of all color intensities and the y-axis represents the
    frequency each intensity occurs in the image.
    The function returns a high value if there are a lot of different color
    intensities in the image, and a low value if there are a lot of similar
    color intensities.
    """

    # Get the distribution of colors in the image.
    # 
    # That is the number of pixels that have colors in each of a fixeed list of
    # color ranges, that span the image's color space, the set of all possible
    # colors.
    ...
    #
    # A bin holds the number of pixels for a intensity range of a color channel.
    # Python's PIL divides a color channel into 256 intensity ranges.
...
def scale_image(image_path, max_width, max_height, crop=True):
    """
    Scale image, optionally cropping it to force the image to the specified
    width and height.

    .. function:: scale_image(image_path, max_width, max_height[, crop=False])
    """
